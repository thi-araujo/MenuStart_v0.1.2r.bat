...
:adjustUac
::Este script ajusta o controle de usuário para o nível 1
...
::Informa um possível mal funcionamento do script
cls
echo Nenhum dos valores determinados foram encontrados, verificar um possível erro do execução do script!
timeout /t 5 >nul 2>&1
exit /b

::Verifica em qual nível está o controle usuário e ajusta para o nível 1
:n3
if '%promptOnSecureDesktopN3%' EQU '1' (
	...
	cls
	echo.
	echo Ajustando o controle de usuário para o nível 1
	...
	echo 1-O controle de usuário foi ajustado para o nivel 1, realizar o logoff >> %tmp%\logoff.txt
	timeout /t 5 >nul 2>&1
	cls
	echo.
	echo Reiniciando a verificação para confirmar que o controle de usuário foi ajustado.
	timeout /t 5 >nul 2>&1
	goto adjustUac
	)
...
if '%promptOnSecureDesktopN1%' EQU '1' (
	echo.
	echo O controle de usuário já está ajustado para o nível 1
	timeout /t 5 >nul 2>&1
	exit /b
	)
...
:n0
	...
	echo.
	echo O controle de usuário está desativado
	timeout /t 5 >nul 2>&1
	cls
	echo.
	echo Ajustando o controle de usuário para o nível 1
